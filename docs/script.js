...
let issueCounts = { forward: 0, slouch: 0, headTilt: 0, shoulderTilt: 0 }; // 問題の検出回数
...
button.addEventListener("click", async function () {
  ...
  try {
    ...
    sessionBadMs = 0;
    lastFrameTime = 0;
    issueCounts = { forward: 0, slouch: 0, headTilt: 0, shoulderTilt: 0 };
    startCalibration();
    detectLoop();
...
function showSessionResult() {
  const goodSec = sessionGoodMs / 1000;
  const badSec = sessionBadMs / 1000;
  ...
  const score = Math.round(goodSec / total * 100);
  const panel = document.getElementById("sessionPanel");
  const scoreEl = document.getElementById("sessionScore");
  const goodEl = document.getElementById("sessionGood");
  const badEl = document.getElementById("sessionBad");
  ...
  goodEl.textContent = formatTime(goodSec);
  badEl.textContent = formatTime(badSec);
  totalEl.textContent = formatTime(total);

  if (score >= 90) {
    msgEl.textContent = "素晴らしい！とても良い姿勢でした。";
    scoreEl.style.color = "#34d399";
  } else if (score >= 70) {
    msgEl.textContent = "なかなか良いです。少し意識するともっと良くなります。";
    scoreEl.style.color = "#fbbf24";
  } else {
    msgEl.textContent = "姿勢が崩れがちでした。こまめに意識してみましょう。";
    scoreEl.style.color = "#f87171";
  }

  panel.style.display = "block";
  setStatus("idle", "セッション終了");
}

/**
 * AI コーチングボタンが押されたときに呼ばれる
 * セッションデータを FastAPI 経由で Gemini API に送り、アドバイスを表示する
 */
async function requestCoaching() {
  ...
  try {
    const res = await fetch(API_BASE + "/coach", {
      ...
      body: JSON.stringify({
        good_seconds: sessionGoodMs / 1000,
        bad_seconds: sessionBadMs / 1000,
        issues: issueCounts,
      }),
    });
    ...
    coachResult.textContent = "サーバーに接続できません。バックエンド（uvicorn）が起動しているか確認してください。";
    coachResult.style.display = "block";
  }

  coachBtn.disabled = false;
  coachBtn.textContent = "AI コーチングを受ける";
}

/** セッション記録を FastAPI に送信する（サーバーが起動していない場合は無視） */
async function sendSessionRecord() {
  const goodSec = sessionGoodMs / 1000;
  const badSec = sessionBadMs / 1000;
  if (goodSec + badSec < 1) return;

  try {
    await fetch(API_BASE + "/records", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ good_seconds: goodSec, bad_seconds: badSec }),
    });
    loadHistory();
  } catch (e) {
    // サーバーが起動していなくてもフロントは正常に動作する
  }
}
...
async function loadHistory() {
  ...
  try {
    ...
    for (const r of recent) {
      ...
      bar.style.backgroundColor = r.score >= 90 ? "#34d399" : r.score >= 70 ? "#fbbf24" : "#f87171";
      bar.title = r.score + "点";
    ...
    for (const r of records.slice(0, 10)) {
      ...
      const total = r.good_seconds + r.bad_seconds;
      const color = r.score >= 90 ? "#34d399" : r.score >= 70 ? "#fbbf24" : "#f87171";

      item.innerHTML =
        '<div><span class="history-date">' + dateStr + '</span>' +
        '<br><span class="history-time">' + formatTime(total) + '</span></div>' +
        '<span class="history-score" style="color:' + color + '">' + r.score + '点</span>';
      list.appendChild(item);
    }
  } catch (e) {
    chart.innerHTML = "";
    list.innerHTML = "";
    empty.textContent = "サーバーに接続できません";
    empty.style.display = "block";
  }
}
